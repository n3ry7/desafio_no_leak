...
def create_custom_colormap(alpha=0.45):
    """Create a custom RGBA colormap with specified transparency."""
    colormap = np.zeros((256, 4), dtype=np.uint8)
    
    # Define color transitions in BGR order
    blue = np.array([255, 0, 0, 255])
    green = np.array([0, 255, 0, 255])
    yellow = np.array([0, 255, 255, 255])
    orange = np.array([0, 165, 255, 255])
    ...
    for i in range(256):
        if i <= thrs1:
            ratio = i / thrs1
            colormap[i] = blue * (1 - ratio) + green * ratio
        elif i <= thrs2:
            ratio = (i - thrs1) / (thrs2 - thrs1)
            colormap[i] = green * (1 - ratio) + yellow * ratio
        elif i <= thrs3:
            ratio = (i - thrs2) / (thrs3 - thrs2)
            colormap[i] = yellow * (1 - ratio) + orange * ratio
        elif i <= thrs4:
            ratio = (i - thrs3) / (thrs4 - thrs3)
            colormap[i] = orange * (1 - ratio) + red * ratio
        else:
            colormap[i] = red
        
        # Set alpha: 0 for 0, alpha_value otherwise
        colormap[i, 3] = 0 if i == 0 else alpha_value
...
def overlay_heatmap(rgb_image, heatmap_rgba):
    ...
    image_rgba = cv2.cvtColor(rgb_image, cv2.COLOR_BGR2BGRA)
    # Normalize alpha channel
    alpha = heatmap_rgba[:, :, 3] / 255.0
    alpha = np.expand_dims(alpha, axis=-1)
    # Blend the images
    blended = (image_rgba * (1 - alpha) + heatmap_rgba * alpha).astype(np.uint8)
